...
class CoordinateSelectorWidget(QWidget):
    ...
    def start_selection(self):
        ...
        if not self.target_window_hwnd:
            ...
            if bound_hwnd:
                self.target_window_hwnd = bound_hwnd
                logger.info(f"靶心 直接获取到窗口句柄: {self.target_window_hwnd}")
            else:
                # 如果获取句柄失败，尝试通过窗口标题查找
                ...
                if window_title:
                    ...
                    try:
                        from utils.window_finder import WindowFinder
                        finder = WindowFinder()
                        self.target_window_hwnd = finder.find_window_by_title(window_title)
                        if self.target_window_hwnd:
                            logger.info(f"靶心 通过窗口标题获取到句柄: 标题={window_title}, 句柄={self.target_window_hwnd}")
                        else:
                            logger.error(f"无法通过标题找到窗口句柄: {window_title}")
                            QMessageBox.warning(self, "警告", f"无法找到窗口: {window_title}")
                            self.selection_finished.emit()
                            return
                    except Exception as e:
                        logger.error(f"查找窗口句柄失败: {e}")
                        QMessageBox.warning(self, "警告", f"查找窗口失败: {e}")
                        self.selection_finished.emit()
                        return
                else:
        ...
        # 验证窗口句柄的类型
        if not isinstance(self.target_window_hwnd, int):
            logger.error(f"错误：窗口句柄类型不正确，期望int，实际{type(self.target_window_hwnd)}: {self.target_window_hwnd}")
            QMessageBox.critical(self, "错误", f"窗口句柄类型错误")
            self.selection_finished.emit()
            return
        ...
        try:
            ...
            logger.error(f"创建坐标选择覆盖层失败: {e}", exc_info=True)
            QMessageBox.critical(self, "错误", f"创建坐标选择器失败: {e}")
            self.selection_finished.emit()
