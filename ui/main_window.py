...
class GlobalSettingsDialog(QDialog):
    ...
    PRESET_INTERNAL_MAP = {v: k for k, v in PRESET_DISPLAY_MAP.items()}
    ...
    def __init__(self, current_config: dict, parent=None):
        ...
        # 🔧 修复:新打开程序时不加载历史绑定窗口,从空列表开始
        self.bound_windows = []
        self.window_binding_mode = current_config.get('window_binding_mode', 'single')
        
        logger.info(f"GlobalSettingsDialog初始化: 从空列表开始,不加载历史绑定窗口")
        ...
        self.batch_add_button.setFixedWidth(80)
        ...
        # 🎯 新增：直接嵌入瞄准器图标
        drag_icon_layout = QHBoxLayout()
        drag_icon_label = QLabel("拖拽绑定:")
        drag_icon_label.setFixedWidth(80)
        
        # 创建瞄准器标签
        from .window_selector_tool import WindowSelectorWidget
        self.window_selector_widget = WindowSelectorWidget(self)
        self.window_selector_widget.window_selected.connect(self._on_window_selected_by_drag)
        
        drag_icon_layout.addWidget(drag_icon_label)
        drag_icon_layout.addWidget(self.window_selector_widget)
        drag_icon_layout.addStretch(1)
        window_layout.addLayout(drag_icon_layout)
        ...
        # --- Execution Mode Group (BindWindowEx预设方案) ---
        self.exec_mode_group = QGroupBox("窗口绑定模式")
        ...
        preset_label = QLabel("绑定方案:")
        preset_label.setFixedWidth(80)
        self.preset_combo = QComboBox()
        self.preset_combo.addItems(list(self.PRESET_DISPLAY_MAP.values()))
        
        # 加载当前预设
        current_preset = current_config.get('bind_preset', 'FOREGROUND')
        display_preset = self.PRESET_DISPLAY_MAP.get(current_preset, "前台模式")
        self.preset_combo.setCurrentText(display_preset)
        
        preset_select_layout.addWidget(preset_label)
        preset_select_layout.addWidget(self.preset_combo)
        exec_mode_layout.addLayout(preset_select_layout)

        # 自定义参数配置面板（初始隐藏）
        self.custom_params_widget = QWidget()
        custom_params_layout = QVBoxLayout(self.custom_params_widget)
        custom_params_layout.setContentsMargins(0, 5, 0, 0)
        custom_params_layout.setSpacing(5)

        # display参数
        display_layout = QHBoxLayout()
        display_layout.addWidget(QLabel("屏幕捕获:"))
        self.display_combo = QComboBox()
        self.display_combo.addItems(["normal", "gdi", "dx"])
        self.display_combo.setCurrentText(current_config.get('bind_display', 'normal'))
        display_layout.addWidget(self.display_combo)
        custom_params_layout.addLayout(display_layout)

        # mouse参数
        mouse_layout = QHBoxLayout()
        mouse_layout.addWidget(QLabel("鼠标模式:"))
        self.mouse_combo = QComboBox()
        self.mouse_combo.addItems(["normal", "windows", "dx.state|dx.input", "dx.state|dx.raw|dx.input|dx.position|dx.focus"])
        self.mouse_combo.setCurrentText(current_config.get('bind_mouse', 'normal'))
        mouse_layout.addWidget(self.mouse_combo)
        custom_params_layout.addLayout(mouse_layout)

        # keypad参数
        keypad_layout = QHBoxLayout()
        keypad_layout.addWidget(QLabel("键盘模式:"))
        self.keypad_combo = QComboBox()
        self.keypad_combo.addItems(["normal", "windows", "dx.state|dx.input", "dx.state|dx.raw|dx.input"])
        self.keypad_combo.setCurrentText(current_config.get('bind_keypad', 'normal'))
        keypad_layout.addWidget(self.keypad_combo)
        custom_params_layout.addLayout(keypad_layout)

        # public参数
        public_layout = QHBoxLayout()
        public_layout.addWidget(QLabel("公共属性:"))
        self.public_combo = QComboBox()
        self.public_combo.addItems(["", "public.hide.dll", "public.memory.drv", "public.hide.dll|public.memory.drv"])
        self.public_combo.setCurrentText(current_config.get('bind_public', ''))
        public_layout.addWidget(self.public_combo)
        custom_params_layout.addLayout(public_layout)

        # mode参数
        mode_layout = QHBoxLayout()
        mode_layout.addWidget(QLabel("绑定模式:"))
        self.mode_spinbox = QSpinBox()
        self.mode_spinbox.setRange(0, 2)
        self.mode_spinbox.setValue(current_config.get('bind_mode_ex', 0))
        self.mode_spinbox.setToolTip("0=推荐, 1=驱动级, 2=VT模式")
        mode_layout.addWidget(self.mode_spinbox)
        mode_layout.addStretch()
        custom_params_layout.addLayout(mode_layout)

        exec_mode_layout.addWidget(self.custom_params_widget)
        
        # 根据当前选择显示/隐藏自定义面板
        self.custom_params_widget.setVisible(current_preset == 'custom')
        ...
        # --- Dual Mode Selection Group ---
        self.dual_mode_group = QGroupBox("双模式选择")
        dual_mode_layout = QVBoxLayout(self.dual_mode_group)
        dual_mode_layout.setSpacing(8)
        dual_mode_layout.setContentsMargins(15, 10, 15, 10)

        # 说明文字
        dual_mode_info = QLabel("选择Python实现或VU插件实现方式")
        dual_mode_info.setStyleSheet("color: #666666; font-size: 9pt;")
        dual_mode_layout.addWidget(dual_mode_info)

        # 模式选择单选按钮
        mode_select_layout = QHBoxLayout()
        self.mode_button_group = QButtonGroup(self)
        
        self.mode_auto = QRadioButton("自动模式 (推荐)")
        self.mode_vu = QRadioButton("VU插件模式")
        self.mode_python = QRadioButton("Python模式")
        
        self.mode_button_group.addButton(self.mode_auto, 0)
        self.mode_button_group.addButton(self.mode_vu, 1)
        self.mode_button_group.addButton(self.mode_python, 2)
        
        mode_select_layout.addWidget(self.mode_auto)
        mode_select_layout.addWidget(self.mode_vu)
        mode_select_layout.addWidget(self.mode_python)
        mode_select_layout.addStretch()
        
        dual_mode_layout.addLayout(mode_select_layout)
        
        # 设置当前选择
        current_dual_mode = current_config.get('dual_execution_mode', 'auto')
        if current_dual_mode == 'vu':
            self.mode_vu.setChecked(True)
        elif current_dual_mode == 'python':
            self.mode_python.setChecked(True)
        else:
            self.mode_auto.setChecked(True)
        
        main_layout.addWidget(self.dual_mode_group)

        # --- Floating Card Display Group ---
        self.floating_card_group = QGroupBox("窗口磁吸工作流")
        floating_card_layout = QVBoxLayout(self.floating_card_group)
        floating_card_layout.setSpacing(8)
        floating_card_layout.setContentsMargins(15, 10, 15, 10)

        # 说明文字
        floating_info = QLabel("在绑定窗口上显示当前执行的任务卡片")
        floating_info.setStyleSheet("color: #666666; font-size: 9pt;")
        floating_card_layout.addWidget(floating_info)

        # 启用开关
        self.enable_floating_card = QCheckBox("启用悬浮卡片显示")
        self.enable_floating_card.setChecked(current_config.get('enable_floating_card', False))
        floating_card_layout.addWidget(self.enable_floating_card)

        main_layout.addWidget(self.floating_card_group)
        ...
        license_layout.setContentsMargins(15, 10, 15, 10)
        
        self.reg_code_edit = QLineEdit()
        self.reg_code_edit.setPlaceholderText("请输入VU插件注册码")
        vu_reg = current_config.get('vu_registration', {})
        self.reg_code_edit.setText(vu_reg.get('reg_code', ''))
        
        self.addon_code_edit = QLineEdit()
        self.addon_code_edit.setPlaceholderText("请输入附加码（可选）")
        self.addon_code_edit.setText(vu_reg.get('addon_code', ''))
        
        license_layout.addRow("注册码:", self.reg_code_edit)
        license_layout.addRow("附加码:", self.addon_code_edit)
    ...
    def _on_preset_changed(self, preset_text):
        """处理预设方案切换，显示/隐藏自定义参数面板"""
        is_custom = (preset_text == "自定义")
    ...
    def _open_window_selector(self):
        """打开窗口选择器工具"""
        try:
            from .window_selector_tool import WindowSelectorTool
            
            selector = WindowSelectorTool(self)
            selector.window_selected.connect(self._on_window_selected_by_drag)
            selector.show()
            
        except Exception as e:
            logger.error(f"打开窗口选择器失败: {e}")
            QMessageBox.warning(self, "错误", f"打开窗口选择器失败: {e}")

    def _on_window_selected_by_drag(self, hwnd: int, title: str):
        """处理拖拽选择的窗口"""
        try:
            logger.info(f"拖拽选择窗口: {title} (HWND: {hwnd})")
            ...
            QMessageBox.information(self, "绑定成功", f"成功绑定窗口:\n{title}\n(句柄: {hwnd})")
            
        except Exception as e:
            logger.error(f"处理拖拽选择的窗口失败: {e}")
            QMessageBox.warning(self, "错误", f"绑定窗口失败: {e}")

        # 🔧 修复:启动时不自动验证和绑定窗口,只加载配置显示
        # 用户打开全局设置时,只显示配置中的窗口信息,不主动查找和绑定
        self._load_bound_windows_passive()
    ...
    def _load_bound_windows_passive(self):
        """被动加载已绑定的窗口列表,仅显示配置信息,不主动验证和绑定"""
        logger.info(f"被动加载绑定窗口配置，共 {len(self.bound_windows)} 个窗口")
        
        # 仅刷新界面显示配置中的窗口信息,不进行任何主动验证或绑定操作
        self._refresh_bound_windows_combo()
        
        logger.info("被动加载完成,窗口信息已显示,未进行主动绑定")
    ...
    def _load_bound_windows(self):
        ...
        # 🔧 应用窗口过滤，排除应用程序自身窗口和无效窗口
        from utils.window_filter import is_valid_target_window
        filtered_windows = [w for w in valid_windows if is_valid_target_window(w.get('hwnd', 0))]
        logger.info(f"过滤后窗口数量: {len(filtered_windows)}")

        # 更新绑定窗口列表为过滤后的列表
        self.bound_windows = filtered_windows
    ...
    def _on_accept(self):
        ...
        try:
            ...
            # 🔧 保存BindWindowEx预设或自定义参数
            selected_preset_text = self.preset_combo.currentText()
            selected_preset = self.PRESET_INTERNAL_MAP.get(selected_preset_text, 'FOREGROUND')

            if selected_preset == 'custom':
                # 保存自定义参数
                self.current_config['bind_preset'] = 'custom'
                self.current_config['bind_display'] = self.display_combo.currentText()
                self.current_config['bind_mouse'] = self.mouse_combo.currentText()
                self.current_config['bind_keypad'] = self.keypad_combo.currentText()
                self.current_config['bind_public'] = self.public_combo.currentText()
                self.current_config['bind_mode_ex'] = self.mode_spinbox.value()
                logger.info(f"  保存自定义BindWindowEx参数")
            else:
                # 保存预设名称并加载预设参数
                self.current_config['bind_preset'] = selected_preset
                from config.vu_config import VUBindPresets
                preset_params = VUBindPresets.get_preset(selected_preset)
                self.current_config['bind_display'] = preset_params['display']
                self.current_config['bind_mouse'] = preset_params['mouse']
                self.current_config['bind_keypad'] = preset_params['keypad']
                self.current_config['bind_public'] = preset_params['public']
                self.current_config['bind_mode_ex'] = preset_params['mode']
                logger.info(f"  保存预设方案: {selected_preset}")

            # 🔧 保存双模式选择
            if self.mode_auto.isChecked():
                self.current_config['dual_execution_mode'] = 'auto'
            elif self.mode_vu.isChecked():
                self.current_config['dual_execution_mode'] = 'vu'
            else:
                self.current_config['dual_execution_mode'] = 'python'

            # 🔧 保存悬浮卡片配置
            self.current_config['enable_floating_card'] = self.enable_floating_card.isChecked()
            ...
            logger.info(f"  已更新 current_config['bind_preset']: {self.current_config['bind_preset']}")
            logger.info(f"  已更新 current_config['dual_execution_mode']: {self.current_config['dual_execution_mode']}")

            # 🔧 保存许可证信息
            self.current_config['vu_registration'] = {
                'reg_code': self.reg_code_edit.text().strip(),
                'addon_code': self.addon_code_edit.text().strip()
            }
            logger.info(f"  已更新许可证信息")

            # 保存配置（这会更新文件）
            self._save_bound_windows_config()

            # 应用双模式选择到DualModeManager
            try:
                from core.dual_mode_manager import set_execution_mode
                set_execution_mode(self.current_config['dual_execution_mode'])
                logger.info(f"[OK] 双模式已切换到: {self.current_config['dual_execution_mode']}")
            except Exception as e:
                logger.warning(f"应用双模式选择失败: {e}")
    ...
    def get_execution_mode(self):
        """获取执行模式（已废弃，保留用于兼容性）"""
        # 现在使用 BindWindowEx 预设方案，不再有单独的执行模式选择
        return 'background'  # 默认返回后台模式
    ...
    def get_settings(self) -> dict:
        ...
        # 获取用户选择的 BindWindowEx 预设方案
        selected_preset_text = self.preset_combo.currentText()
        selected_preset = self.PRESET_INTERNAL_MAP.get(selected_preset_text, 'FOREGROUND')
        
        # 根据预设方案确定执行模式（后台预设使用后台模式，前台预设使用前台模式）
        if selected_preset == 'FOREGROUND':
            internal_mode = 'foreground'
        else:
            internal_mode = 'background'
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, task_modules: Dict[str, Any], initial_config: dict, hardware_id: str, license_key: str, save_config_func, images_dir: str, task_state_manager=None):
        ...
        self.current_target_window_title = None  # 🔧 修复:新打开软件时不显示上次的目标窗口
