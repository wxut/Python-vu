...
try:
    ...
    from adapters import VUImageAdapter

    _vu = get_global_vu()
    _image_adapter = VUImageAdapter(_vu)
    VU_IMAGE_AVAILABLE = True
    logger.debug("VU 图像处理功能可用")
except Exception as e:
    VU_IMAGE_AVAILABLE = False
    _image_adapter = None
    logger.warning(f"VU 图像处理不可用: {e}")
# --- END ADDED ---
...
class ImageOperations:
    ...
    @staticmethod
    def find_image_vu(image_path: str, region: Optional[Tuple[int, int, int, int]] = None,
        ...
        Returns:
            Optional[Tuple[int, int]]: 找到的位置(x, y)，未找到返回None
        """
        if not VU_IMAGE_AVAILABLE or _image_adapter is None:
            logger.warning("VU图像功能不可用，无法使用VU查找图像")
            return None

        try:
            logger.debug(f"使用VU查找图像: {image_path}, region={region}, confidence={confidence}")
            pos = _image_adapter.find_image(image_path, region=region, threshold=confidence)
            if pos:
                logger.debug(f"VU找到图像: {pos}")
            return pos
        except Exception as e:
            logger.error(f"VU查找图像失败: {e}")
            return None
...
def find_image_vu(image_path: str, region: Optional[Tuple[int, int, int, int]] = None,
                  confidence: float = 0.8) -> Optional[Tuple[int, int]]:
    """兼容性函数 - 使用VU查找图像"""
    return ImageOperations.find_image_vu(image_path, region, confidence)
